...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(description='2D Lennard-Jones Particle Simulation (NVE)')
    ...
    parser.add_argument('--vis_steps', type=int, default=10, help='Visualization update frequency')
    parser.add_argument('--rdf_bins', type=int, default=50, help='Number of bins for RDF')
    parser.add_argument('--thermostat', action='store_true', help='Enable Berendsen thermostat')
    parser.add_argument('--tau_factor', type=float, default=0.0025, help='dt/τ factor for Berendsen thermostat')
...
def main():
    ...
    for step in range(warmup_steps):
        sim.step()
        if step % 10 == 0:
            print(f"Warmup step {step}/{warmup_steps}, T={sim.temperature:.3f}, "
    ...
    for step in range(args.steps):
        ...
        if step % 100 == 0:
            ...
            print(f"Step {step}/{args.steps}, T={sim.temperature:.3f}, "
