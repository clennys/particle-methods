...
def run_nve_simulation(N, L=30.0, dt=0.01, steps=1000, temp=0.5, rc=2.5):
    ...
    for step in range(steps):
        ...
        if step % 100 == 0:
            ...
            energy_drift = (sim.total_energy - initial_energy) / abs(initial_energy)
            print(f"Step {step}/{steps}, T={sim.temperature:.3f}, "
                  f"E_tot={sim.total_energy:.5f}, Drift={energy_drift:.6%}, |P|={momentum:.6f}")
...
def compare_time_steps(N=100, steps=1000, time_steps=[0.001, 0.005, 0.01, 0.02]):
    ...
    for dt in time_steps:
        ...
        print(f"{dt:<10} {stats['rel_fluctuation']:<15.6%} {stats['rel_drift']:<15.6%} {stats['steps_per_sec']:<10.1f}")
...
def compare_system_sizes(dt=0.01, steps=1000, sizes=[100, 225, 400, 625, 900]):
    ...
    # Print comparison table
    print("\n=== Energy Conservation Comparison ===\n")
    print(f"{'System Size':<12} {'Rel. Fluct.':<15} {'Rel. Drift':<15} {'Steps/sec':<10}")
    print("-" * 52)
    
    for N in sizes:
        stats = results[N]['stats']
        print(f"{N:<12} {stats['rel_fluctuation']:<15.6%} {stats['rel_drift']:<15.6%} {stats['steps_per_sec']:<10.1f}")
