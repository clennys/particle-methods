...
class Simulation:
    ...
    def calculate_distance(self, agent1, agent2):
        dx = min(
            abs(agent1.x - agent2.x),
            abs(agent1.x - agent2.x - self.domain_size),
            abs(agent1.x - agent2.x + self.domain_size),
        )
        dy = min(
            abs(agent1.y - agent2.y),
        ...
        for _, wolf in self.wolves.items():
            ...
            for rabbit_id in nearby_rabbit_ids:
                ...
                if (
                    ...
                    and np.random.random() < self.eat_prob
                ):
                    rabbits_eaten.add(rabbit_id)
                    wolf.hunger = 0

                    if np.random.random() < self.eat_prob:
        ...
        if plot_interval is not None:
            ...
            def update(frame):
                ...
                if frame % plot_interval == 0:
                    rabbit_positions = np.array(
                        [[r.x, r.y] for r in self.rabbits.values()]
                    )
                    wolf_positions = np.array(
                        [[w.x, w.y] for w in self.wolves.values()]
                    )
            ...
            for t in range(time_steps):
                if t % 100 == 0:
                    ...
                    print(
                        f"Time step: {t}, Rabbits: {len(self.rabbits)}, Wolves: {len(self.wolves)}, Elapsed: {elapsed:.2f}s"
                    )
                self.step()
