...
class SimulationAnalyzer:
            ...
            if len(progression_data) >= 3:
                try:
                    ...
                    for snapshot in progression_data:
                        ...
                        if area > 0:
                            edges = ndimage.sobel(burned_area)
                            perimeter = np.sum(edges > 0)
                            perimeters.append(perimeter)
                            areas.append(area)

                    if areas and perimeters:
                        # Plot area vs perimeter (fire shape complexity)
                        axes[1, 2].scatter(areas, perimeters, color="purple", alpha=0.7)
    ...
    def quick_summary(self):
        """Print a quick text summary of key results"""
        print("\n" + "=" * 60)
        ...
        # Calculate missing stats from the DataFrame if not in summary_stats
        peak_burn_rate = self.summary_stats.get(
            "peak_burning_cells",
            max(self.df["burn_rate"]) if "burn_rate" in self.df.columns else 0,
        )
        ...
        avg_intensity = self.summary_stats.get(
            ...
            (
                np.mean(
                    [x for x in self.df.get("particle_intensity_avg", [0]) if x > 0]
                )
                if "particle_intensity_avg" in self.df.columns
                else 0
            ),
        )
...
def compare_simulations(file_list, output_dir="comparison_output"):
    ...
    for analyzer, label in zip(analyzers, labels):
        row = [
            ...
            f"{analyzer.summary_stats['max_burn_rate']:.1f}",
            f"{analyzer.summary_stats['simulation_duration']}",
            f"{analyzer.summary_stats['max_particles']}",
        ]
