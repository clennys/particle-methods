...
def setup_couette_flow():
    ...
    top_wall = np.where((sim.types == sim.WALL) & (sim.positions[:,1] > sim.L - sim.rc))[0]
    
    sim.velocities[bottom_wall] = [5.0, 0.0]
    sim.velocities[top_wall] = [-5.0, 0.0]
    
    print(f"Created Couette flow setup with {len(bottom_wall)} bottom wall and {len(top_wall)} top wall particles")
...
def run_simulation(sim, args):
    ...
    for step in range(args.steps):
        ...
        if step % 100 == 0:
            print(f"Step {step}/{args.steps}, T={sim.temperature:.3f}, t={sim.time:.2f}")
            ...
            if args.scenario == "couette" and step % 500 == 0:
                ...
                print(f"  Velocity profile (min/max): {np.min(vx_profile):.3f}/{np.max(vx_profile):.3f}")
            
            if args.scenario == "poiseuille" and step % 500 == 0:
                _, density_a = sim.get_density_profile(direction='y', particle_type=sim.TYPE_A)
                print(f"  Type A density profile (min/max): {np.min(density_a):.3f}/{np.max(density_a):.3f}")
    
    elapsed = time.time() - start_time
    steps_per_sec = args.steps / elapsed
    print(f"Simulation completed in {elapsed:.2f} seconds ({steps_per_sec:.1f} steps/sec)")
...
def save_results(sim, args):
    ...
    if args.scenario == "couette":
        ...
        np.savetxt(os.path.join(plots_dir, 'velocity_profile.csv'), 
                  np.column_stack((y_bins, vx_profile)), 
                  delimiter=',', 
                  header='y,vx')
    
    if args.scenario == "poiseuille":
        y_bins, density_f = sim.get_density_profile(direction='y', particle_type=sim.FLUID)
        y_bins, density_a = sim.get_density_profile(direction='y', particle_type=sim.TYPE_A)
        ...
        np.savetxt(os.path.join(plots_dir, 'density_profile.csv'), 
