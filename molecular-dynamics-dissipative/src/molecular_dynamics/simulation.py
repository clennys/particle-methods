...
class DPDSimulation:
    ...
    def __init__(
        ...
        if abs(self.sigma**2 - 2*self.gamma*self.kBT) > 1e-10:
            print("Warning: DPD thermostat relation is not satisfied: sigma^2 = 2*gamma*kBT")
            print(f"  sigma^2 = {self.sigma**2}, 2*gamma*kBT = {2*self.gamma*self.kBT}")
            print(f"  For current gamma={self.gamma} and kBT={self.kBT}, sigma should be {np.sqrt(2*self.gamma*self.kBT)}")
        else:
        ...
        self.types = np.full(self.N_total, self.FLUID, dtype=int)  # Start with all fluid
    ...
    def create_chain_molecules(self, num_chains):
        ...
        if len(available_indices) < num_chains * 7:
            raise ValueError(f"Not enough fluid particles to create {num_chains} chains of 7 particles each")
        ...
        for i in range(num_chains):
            ...
            for j, idx in enumerate(chain_indices):
                ...
                offset = np.array([j * self.r_S * 1.2, 0]) + np.random.uniform(-0.1, 0.1, 2)
    ...
    def create_ring_molecules(self, num_rings, ring_size=9):
        ...
        if len(available_indices) < num_rings * ring_size:
            raise ValueError(f"Not enough fluid particles to create {num_rings} rings of {ring_size} particles each")
        ...
        for i in range(num_rings):
            ...
            radius = self.r_S * ring_size / (2 * np.pi)  # Approximate radius to achieve desired bond length
    ...
    def create_walls(self, thickness=None, positions=None):
        ...
        for wall_pos in positions:
            if wall_pos == 'x=0':
                wall_indices = np.where((self.types == self.FLUID) & (self.positions[:, 0] < thickness))[0]
            elif wall_pos == 'x=L':
                wall_indices = np.where((self.types == self.FLUID) & (self.positions[:, 0] > self.L - thickness))[0]
            elif wall_pos == 'y=0':
                wall_indices = np.where((self.types == self.FLUID) & (self.positions[:, 1] < thickness))[0]
            elif wall_pos == 'y=L':
                wall_indices = np.where((self.types == self.FLUID) & (self.positions[:, 1] > self.L - thickness))[0]
            else:
    ...
    def calculate_forces(self):
        ...
        if self.body_force != 0:
            self.forces = apply_body_force(self.forces, self.types, self.body_force, self.WALL)
    ...
    def step(self):
        ...
        if len(wall_indices) > 0:
            ...
            if np.any(self.wall_velocity != 0):
                ...
                self.positions[wall_indices] = np.mod(self.positions[wall_indices], self.L)
    ...
    def record_measurements(self):
        ...
        self.total_energy_history.append(self.kinetic_energy + self.potential_energy + self.bond_energy)
