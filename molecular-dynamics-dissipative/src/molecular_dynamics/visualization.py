...
class DPDVisualizer:
    ...
    def setup(self):
        ...
        self.ax_sim.set_title(f"DPD Simulation - t={self.sim.time:.2f}, T={self.sim.temperature:.3f}", fontsize=14)
        ...
        for type_id in range(4):
            if np.any(self.sim.types == type_id):
                (line,) = self.ax_density.plot([], [], '.-', color=self.COLORS[type_id], label=self.TYPE_NAMES[type_id])
    ...
    def update(self):
        ...
        for type_id, line in self.density_lines.items():
            y_bins, density = self.sim.get_density_profile(direction='y', particle_type=type_id)
        ...
        if self.density_lines and len(y_bins) > 0:
            ...
            for type_id in self.density_lines:
                _, density = self.sim.get_density_profile(direction='y', particle_type=type_id)
                if len(density) > 0:
    ...
    def plot_final_results(self):
        ...
        ax_snapshot.set_title(f"Final Configuration - t={self.sim.time:.2f}", fontsize=14)
        ...
        y_bins, vx_profile = self.sim.get_velocity_profile(direction='y', component='x', bins=30)
        ...
        for type_id in range(4):
            if np.any(self.sim.types == type_id):
                y_bins, density = self.sim.get_density_profile(direction='y', particle_type=type_id, bins=30)
                ax_density.plot(y_bins, density, '.-', color=self.COLORS[type_id], label=self.TYPE_NAMES[type_id])
        ...
        ax_energy.plot(self.sim.time_history, self.sim.kinetic_energy_history, 'r-', label='Kinetic')
        ax_energy.plot(self.sim.time_history, self.sim.potential_energy_history, 'g-', label='Potential')
        ax_energy.plot(self.sim.time_history, self.sim.total_energy_history, 'b-', label='Total')
        ...
        if self.sim.molecules:
            ...
            for molecule in self.sim.molecules:
                ...
                molecule_positions = np.array([self.sim.positions[idx] for idx in molecule])
                center = np.mean(molecule_positions, axis=0)
                molecule_centers_y.append(center[1])
                
            if molecule_centers_y:
                ax_molecules.hist(molecule_centers_y, bins=20, alpha=0.7, color='purple')
